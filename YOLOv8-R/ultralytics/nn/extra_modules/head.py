...
class Detect_DyHead(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, block_num=2, ch=()):  # detection layer
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                Conv(hidc, c2, 3), Conv(c2, c2, 3), nn.Conv2d(c2, 4 * self.reg_max, 1)
            )
            for _ in ch
        )
...
class Detect_DyHeadWithDCNV3(Detect_DyHead):
    def __init__(self, nc=80, hidc=256, block_num=2, ch=()):
        ...
        self.dyhead = nn.Sequential(
            *[DyHeadBlockWithDCNV3(hidc) for i in range(block_num)]
        )
...
class Detect_DyHeadWithDCNV4(Detect_DyHead):
    def __init__(self, nc=80, hidc=256, block_num=2, ch=()):
        super().__init__(nc, hidc, block_num, ch)
        self.dyhead = nn.Sequential(
            *[DyHeadBlockWithDCNV4(hidc) for i in range(block_num)]
        )
...
class Detect_Efficient(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):  # detection layer
        ...
        self.stem = nn.ModuleList(
            nn.Sequential(Conv(x, x, 3), Conv(x, x, 3)) for x in ch
        )  # two 3x3 Conv
    ...
    def forward(self, x):
        ...
        if self.training:
            for i in range(self.nl, 2 * self.nl):
                x[i] = torch.cat(
                    (self.cv4[i - self.nl](x[i]), self.cv5[i - self.nl](x[i])), 1
                )
            return x
        elif self.dynamic or self.shape != shape:
            ...
            self.anchors, self.strides = (
                x.transpose(0, 1) for x in make_anchors(x[: self.nl], self.stride, 0.5)
            )
            self.shape = shape
...
class Detect_SEAM(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c2, 3), SEAM(c2, c2, 1, 16), nn.Conv2d(c2, 4 * self.reg_max, 1)
            )
            for x in ch
        )
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c3, 3), SEAM(c3, c3, 1, 16), nn.Conv2d(c3, self.nc, 1)
            )
            for x in ch
        )
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
class Detect_MultiSEAM(Detect_SEAM):
    def __init__(self, nc=80, ch=()):
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c2, 3), MultiSEAM(c2, c2, 1), nn.Conv2d(c2, 4 * self.reg_max, 1)
            )
            for x in ch
        )
        self.cv3 = nn.ModuleList(
            nn.Sequential(
                Conv(x, c3, 3), MultiSEAM(c3, c3, 1), nn.Conv2d(c3, self.nc, 1)
            )
            for x in ch
        )
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
class Segment_LSCD(LSCDH):
    ...
    def __init__(self, nc=80, nm=32, npr=256, hidc=256, ch=()):
        ...
        self.npr = npr  # number of protos
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect = LSCDH.forward

        c4 = max(ch[0] // 4, self.nm)
        self.cv4 = nn.ModuleList(
            nn.Sequential(
                Conv_GN(x, c4, 1), Conv_GN(c4, c4, 3), nn.Conv2d(c4, self.nm, 1)
            )
            for x in ch
        )
...
class Pose_LSCD(LSCDH):
    ...
    def __init__(self, nc=80, kpt_shape=(17, 3), hidc=256, ch=()):
        ...
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c4, 1), Conv(c4, c4, 3), nn.Conv2d(c4, self.nk, 1))
            for x in ch
        )
...
class OBB_LSCD(LSCDH):
    ...
    def __init__(self, nc=80, ne=1, hidc=256, ch=()):
        ...
        self.cv4 = nn.ModuleList(
            nn.Sequential(
                Conv_GN(x, c4, 1), Conv_GN(c4, c4, 3), nn.Conv2d(c4, self.ne, 1)
            )
            for x in ch
        )
...
class Detect_TADDH(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        self.share_conv = nn.Sequential(
            Conv_GN(hidc, hidc // 2, 3), Conv_GN(hidc // 2, hidc // 2, 3)
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x[i] = torch.cat(
                (self.scale[i](self.cv2(reg_feat)), self.cv3(cls_feat * cls_prob)), 1
            )
        if self.training:  # Training path
            return x
...
class Detect_LADH(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv(x, c3, 1), Conv(c3, c3, 1), nn.Conv2d(c3, self.nc, 1))
            for x in ch
        )
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
class OBB_LADH(Detect_LADH):
    ...
    def __init__(self, nc=80, ne=1, ch=()):
        ...
        c4 = max(ch[0] // 4, self.ne)
        self.cv4 = nn.ModuleList(
            nn.Sequential(DSConv(x, c4, 3), Conv(c4, c4, 1), nn.Conv2d(c4, self.ne, 1))
            for x in ch
        )
...
class Detect_LSCSBD(nn.Module):
    ...
    def __init__(self, nc=80, hidc=256, ch=()):
        ...
        self.conv = nn.ModuleList(nn.Sequential(Conv(x, hidc, 1)) for x in ch)
        self.share_conv = nn.Sequential(
            nn.Conv2d(hidc, hidc, 3, 1, 1), nn.Conv2d(hidc, hidc, 3, 1, 1)
        )
        self.separate_bn = nn.ModuleList(
            nn.Sequential(nn.BatchNorm2d(hidc), nn.BatchNorm2d(hidc)) for _ in ch
        )
        self.act = nn.SiLU()
        self.cv2 = nn.Conv2d(hidc, 4 * self.reg_max, 1)
        self.cv3 = nn.Conv2d(hidc, self.nc, 1)
...
class Segment_LSCSBD(Detect_LSCSBD):
    ...
    def __init__(self, nc=80, nm=32, npr=256, hidc=256, ch=()):
        ...
        c4 = max(ch[0] // 4, self.nm)
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c4, 1), Conv(c4, c4, 3), nn.Conv2d(c4, self.nm, 1))
            for x in ch
        )
...
class OBB_LSCSBD(Detect_LSCSBD):
    ...
    def __init__(self, nc=80, ne=1, hidc=256, ch=()):
        ...
        c4 = max(ch[0] // 4, self.ne)
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c4, 1), Conv(c4, c4, 3), nn.Conv2d(c4, self.ne, 1))
            for x in ch
        )
