...
from ultralytics.nn.extra_modules.MyPruner import (
    DiverseBranchBlockPruner,
    LayerNormPruner,
    RepConvPruner,
    DyHeadBlockPruner,
    RepConvNPruner,
...
def get_pruner(opt, model, example_inputs):
    ...
    if opt.prune_method == "random":
        ...
        pruner_entry = partial(
            tp.pruner.MagnitudePruner, global_pruning=opt.global_pruning
        )
        ...
        pruner_entry = partial(
            tp.pruner.BNScalePruner, reg=opt.reg, global_pruning=opt.global_pruning
        )
        ...
        pruner_entry = partial(
            tp.pruner.GroupNormPruner, global_pruning=opt.global_pruning
        )
        ...
        imp = tp.importance.GroupNormImportance(p=2)
        pruner_entry = partial(
            tp.pruner.GroupNormPruner, reg=opt.reg, global_pruning=opt.global_pruning
        )
...
linear_trans = (
    lambda epoch, epochs, reg, reg_ratio: (1 - epoch / (epochs - 1)) * (reg - reg_ratio)
    + reg_ratio
)
...
class DetectionCompressor(BaseTrainer):
    ...
    def model_prune(self, imp, prune, example_inputs):
        ...
        with HiddenPrints():
            ori_flops, ori_params = tp.utils.count_ops_and_params(
        ...
        ori_map50, ori_map = (
            ori_result["metrics/mAP50(B)"],
            ori_result["metrics/mAP50-95(B)"],
        )
        ...
        while speed_up < self.args.speed_up:
            ...
            prune_map50, prune_map = (
                prune_result["metrics/mAP50(B)"],
                prune_result["metrics/mAP50-95(B)"],
            )
            with HiddenPrints():
                prune_flops, prune_params = tp.utils.count_ops_and_params(
                    self.model, example_inputs
                )
            prune_flops = prune_flops * 2.0
            prune_flops_f, prune_params_f = clever_format(
                [prune_flops, prune_params], "%.3f"
            )
        ...
        LOGGER.info(
            f"model params:{ori_params_f} => {prune_params_f}({prune_params / ori_params * 100:.2f}%)"
        )
        LOGGER.info(
            f"model map@50:{ori_map50:.3f} => {prune_map50:.3f}({prune_map50 - ori_map50:.3f})"
        )
        LOGGER.info(
            f"model map@50:95:{ori_map:.3f} => {prune_map:.3f}({prune_map - ori_map:.3f})"
        )
        ...
        for epoch in range(self.start_epoch, self.epochs):
                ...
                if isinstance(prune, (tp.pruner.BNScalePruner,)):
                    if self.args.reg_decay_type == "linear":
                        reg = linear_trans(
                            ...
                            self.epochs,
                            self.args.reg,
                            self.args.reg * self.args.reg_decay,
                        )
                    elif self.args.reg_decay_type == "step":
                        reg = self.args.reg * (
                            self.args.reg_decay ** (epoch // self.args.reg_decay_step)
                        )
                    elif self.args.opt.reg_decay_type == "constant":
                        reg = self.args.reg
                    prune.regularize(self.model, reg=reg)
                elif isinstance(
                    prune, (tp.pruner.GroupNormPruner, tp.pruner.GrowingRegPruner)
                ):
                    reg = self.args.reg
            ...
            for name in model_sl:
                if "weight" in name and len(model_sl[name].size()) == 1:
                    weight = (
                        model_sl[name].data.cpu().abs().clone().numpy().reshape((-1))
                    )
            ...
            plt.tight_layout()
            plt.savefig(
                f"{self.save_dir}/visual/{epoch}_sl_{self.sparsity_ratio:.3f}.png"
            )
            plt.close("all")
            LOGGER.info(
    ...
    def compress(self):
        ...
        torch.save(
            {"model": deepcopy(self.model).half(), "ema": None},
            self.wdir / "model_c2f_v2.pt",
        )
        ...
        if type(self.args.imgsz) is int:
            example_inputs = torch.randn((1, 3, self.args.imgsz, self.args.imgsz)).to(
                self.device
            )
        elif type(self.args.imgsz) is list:
            example_inputs = torch.randn(
                (1, 3, self.args.imgsz[0], self.args.imgsz[1])
            ).to(self.device)
        else:
        ...
        sparsity_learning, imp, prune = get_pruner(
            self.args, self.model, example_inputs
        )
        ...
        LOGGER.info(
            colorstr(
                f"Pruning after Finetune before the model is saved in:{prune_path}"
            )
        )
...
class DetectionFinetune(yolo.detect.DetectionTrainer):
    ...
    def setup_model(self):
        ...
        self.model = self.get_model(
            cfg=model, weights=weights, verbose=RANK == -1
