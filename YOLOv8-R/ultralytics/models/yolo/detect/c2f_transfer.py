...
def transfer_weights_c2f_v2_to_c2f(c2f_v2, c2f):
    ...
    new_cv1 = Conv(
        ...
        c2=(
            state_dict_v2["cv0.conv.weight"].size()[0]
            + state_dict_v2["cv1.conv.weight"].size()[0]
        ),
    ...
    for bn_key in ["weight", "bias", "running_mean", "running_var"]:
        state_dict[f"cv1.bn.{bn_key}"] = torch.cat(
            [state_dict_v2[f"cv0.bn.{bn_key}"], state_dict_v2[f"cv1.bn.{bn_key}"]],
            dim=0,
        )
