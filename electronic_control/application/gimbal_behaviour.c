...
static void gimbal_auto_search(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set);

//云台行为状态机
 gimbal_behaviour_e gimbal_behaviour = GIMBAL_ZERO_FORCE;

gimbal_behaviour_e gimbal_last_behaviour = GIMBAL_ZERO_FORCE;
...
void gimbal_behaviour_mode_set(gimbal_control_t* gimbal_mode_set)
{
    ...
    //云台行为状态机设置
		static int remote_buzzer_flag = 200;
    ...
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode = GIMBAL_MOTO_DOWN;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTO_DOWN;
    }
...
/**
  ...
  * @retval         none
  */
void gimbal_behaviour_control_set(fp32* add_yaw, fp32* add_pitch, fp32 *add_updown, gimbal_control_t* gimbal_control_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_REMOTE)
    {
        gimbal_absolute_angle_control(add_yaw, add_pitch, gimbal_control_set);
				if(gimbal_control_set->gimbal_rc_ctrl->rc.ch[4]>200)
				{
					gimbal_control_set->gimbal_updown_motor1.move_flag = 1;
				}
				else if(gimbal_control_set->gimbal_rc_ctrl->rc.ch[4]<-200)
				{
					gimbal_control_set->gimbal_updown_motor1.move_flag = -1;
				}
				else
				{
					gimbal_control_set->gimbal_updown_motor1.move_flag = 0;
				}
    }
    else if (gimbal_behaviour == GIMBAL_AUTO)
    {
...
static void gimbal_behavour_set(gimbal_control_t* gimbal_mode_set)//祖传拼写 勿动
{
    if (gimbal_mode_set == NULL)
    {
        return;
...

    {
        ...
        if (last_gimbal_behaviour == GIMBAL_REMOTE && gimbal_behaviour != GIMBAL_REMOTE)
        {
          if(gimbal_mode_set->gimbal_yaw_motor.last_remote_mote_mode == GIMBAL_MOTOR_GYRO)
          {
            gimbal_mode_set->gimbal_yaw_motor.last_remote_mote_mode = GIMBAL_MOTOR_ENCONDE;
          }
          else if(gimbal_mode_set->gimbal_yaw_motor.last_remote_mote_mode == GIMBAL_MOTOR_ENCONDE)
          {
            gimbal_mode_set->gimbal_yaw_motor.last_remote_mote_mode = GIMBAL_MOTOR_GYRO;
          }
        }
        if (last_gimbal_behaviour != GIMBAL_AUTO && gimbal_behaviour == GIMBAL_AUTO)
        {
            autoaim_target_reset();
						gimbal_mode_set->auto_gimbal_mode = STEP1_;
						gimbal_mode_set->auto_gimbal_condition = GWAIT;
						grab_reset();
						gimbal_mode_set->auto_grab_count = 0;
        }
...
static void gimbal_init_control(fp32* yaw, fp32* pitch, gimbal_control_t* gimbal_control_set)
{
   ...
   // if(fabs(INIT_PITCH_SET - gimbal_control_set->gimbal_pitch_motor.relative_angle) > GIMBAL_INIT_ANGLE_ERROR)
    //{
        *pitch = (INIT_PITCH_SET - gimbal_control_set->gimbal_pitch_motor.absolute_angle_set) * GIMBAL_INIT_PITCH_SPEED;
        *yaw = 0.0f;
    //}
}
...
static void gimbal_absolute_angle_control(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
    ...
    static int16_t yaw_channel = 0, pitch_channel = 0;
		...
		else if(switch_is_mid(gimbal_control_set->gimbal_rc_ctrl->rc.s[0]))
...
//			if(gimbal_control_set->gimbal_rc_ctrl->rc.ch[REMOTE_CONTROL_GIMBAL_ON_OR_OFF]<REMOTE_CONTROL_GIMBAL_NUM)
//			{
			*yaw = yaw_channel * YAW_RC_SEN / 600;
			*pitch = pitch_channel * PITCH_RC_SEN/-600 ;
...
static void gimbal_auto_search(fp32 *yaw, fp32 *pitch, gimbal_control_t *gimbal_control_set)
{
	static fp32 pitch_add_angle = -SEARCH_PITCH_SPEED;
	*pitch = pitch_add_angle;
	if(gimbal_control_set->gimbal_pitch_motor.absolute_angle_set  > gimbal_control_set->gimbal_pitch_motor.max_absolute_angle - 0.1 )
	{
		pitch_add_angle = -SEARCH_PITCH_SPEED;

	}
	else if(gimbal_control_set->gimbal_pitch_motor.absolute_angle_set  < gimbal_control_set->gimbal_pitch_motor.min_absolute_angle + 0.05 )
	{ 
		pitch_add_angle = SEARCH_PITCH_SPEED;
	}
	*pitch = pitch_add_angle;
	
		static fp32 yaw_add_angle = -SEARCH_YAW_SPEED;
	*yaw = yaw_add_angle;
//	if(gimbal_control_set->gimbal_yaw_motor.absolute_angle  > gimbal_control_set->gimbal_yaw_motor.max_absolute_angle)
//	{
//		yaw_add_angle = -SEARCH_YAW_SPEED;

//	}
//	else if(gimbal_control_set->gimbal_yaw_motor.absolute_angle  < gimbal_control_set->gimbal_yaw_motor.min_absolute_angle)
//	{
//		yaw_add_angle = SEARCH_YAW_SPEED;
//	}
	*yaw = yaw_add_angle;
