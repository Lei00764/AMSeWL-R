...
#include <stdlib.h>
#include <string.h>
int identify_start_flag = 0;
int identify_start_count = 1;

fp32 identify_sin_signal;
...
fp32 give_current_up = -2;
fp32 give_current_down = 0.8;
fp32 given_current = 0;
fp32 give_current_sin =0;
fp32 time_count_right = 0;
int send_data;
int test1m;
int test2m;
int flag_m;
...
extern gimbal_control_t gimbal_control;
  ...
  time_count++; //每1ms加1
	time_count_right = time_count / time_k; //时间
	
	time_count_right = loop_fp32_constrain(time_count_right,0,2*3.14159265358979);
	
	if(time_count_right < 3.14159265358979) {give_current_sin = 1.5*arm_sin_f32(time_count_right);}
	else if(time_count_right > 3.14159265358979) {give_current_sin = 1.5*arm_sin_f32(time_count_right);}
	
	if(switch_is_down(gimbal_control.gimbal_rc_ctrl->rc.s[0]))
	{
				ctrl_motor(YAW_MOTOR_ID,0,0,0,0,0);
	}
	else
	{
		if(gimbal_control.gimbal_yaw_motor.motor_gyro > 0)
		{
			if(give_current_sin>0){flag_m = 1;}
			else{flag_m = 2;}
		}
		else
		{
			if(give_current_sin>0){flag_m = 3;}
			else{flag_m = 4;}
		}
		
		send_data = ((int)(fabs(gimbal_control.gimbal_yaw_motor.motor_gyro) * 1000) + 1)*1000000 + ((int)( fabs(give_current_sin)* 1000))*10 + flag_m;
		test1m = (int)(gimbal_control.gimbal_yaw_motor.motor_gyro*1000);
		test2m = (int)((give_current_sin)* 1000);
//			sprintf(test_mm,"%d-%d",test1m,test2m);
		test_mm[0] = test1m / 1000 + 48; //千位
		test_mm[1] = (test1m / 100) % 10 + 48; //百位
		test_mm[2] = (test1m / 10) % 10 + 48; //十位
		test_mm[3] = (test1m) % 10 + 48; //个位
		
		test_mm[4] = 1;
		
		test_mm[5] = test2m / 1000 + 48; //千位
		test_mm[6] = (test2m / 100) % 10 + 48; //百位
		test_mm[7] = (test2m / 10) % 10 + 48; //十位
		test_mm[8] = (test2m) % 10 + 48; //个位
		
			usart_printf(test_mm);

	//ctrl_motor_pitch(PITCH_MOTOR_ID,0,0,0,0,give_current_sin);
		//ctrl_motor(YAW_MOTOR_ID,0,0,0,0,give_current_sin);
		//UART_DMA_SEND(give_current_sin * 10000);
		//UART_DMA_SEND(gimbal_control.gimbal_pitch_motor.motor_gyro * 10000);
			//UART_DMA_SEND(send_data);
	}
...
//extern gimbal_control_t gimbal_control;
//								if(gimbal_control.gimbal_yaw_motor.given_current < 0)
//								{
//				UART_DMA_SEND(gimbal_control.gimbal_yaw_motor.absolute_angle);
//								}
